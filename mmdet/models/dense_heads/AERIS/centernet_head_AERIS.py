...
class CenterNetHead_AERIS(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
                 ...
                 loss_sr=dict(type='L1Loss', loss_weight=1.0),
                 train_cfg=None,
                 test_cfg=None,
                 init_cfg=None):
        super(CenterNetHead_AERIS, self).__init__(init_cfg)
        self.num_classes = num_classes
        self.heatmap_head = self._build_head(in_channel, feat_channel,
                                             num_classes)
        self.wh_head = self._build_head(in_channel, feat_channel, 2)
        self.offset_head = self._build_head(in_channel, feat_channel, 2)
        self.sr_head = self._build_sr_head(0, in_channel, feat_channel, 48)
        self.loss_center_heatmap = build_loss(loss_center_heatmap)
        self.loss_wh = build_loss(loss_wh)
        self.loss_offset = build_loss(loss_offset)
        self.loss_sr = build_loss(loss_sr)
    ...
    def _build_sr_head(self, q_number, in_channel, feat_channel, out_channel):
        """Build head for SR branch."""
        layer = nn.Sequential(
            nn.Conv2d(in_channel+q_number, feat_channel, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(feat_channel, feat_channel, kernel_size=3),
    ...
    def forward_train(self,
        ...
        if gt_labels is None:
            ...
            loss_inputs = outs + (gt_bboxes, gt_labels, img, img_metas)
    ...
    def forward(self, feats):
        ...
        sr_pred = self.sr_head(feat)
        #print(sr_pred.shape)
        return [center_heatmap_pred], [wh_pred], [offset_pred], [sr_pred]

    @force_fp32(apply_to=('center_heatmap_preds', 'wh_preds', 'offset_preds', 'sr_preds'))
    def loss(self,
        ...
        loss_offset = self.loss_offset(
            ...
            wh_offset_target_weight,
            avg_factor=avg_factor * 2)
        loss_sr = self.loss_sr(sr_pred, img)
        #print(loss_sr)
        return dict(
