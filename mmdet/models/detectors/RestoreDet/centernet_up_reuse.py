...
from .deg_train import deg_resolution, random_deg
...
class CenterNet_Up_Re(SingleStageDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(CenterNet_Up_Re, self).__init__(backbone, neck, bbox_head, train_cfg,
                                           test_cfg, pretrained, init_cfg)
        self.ARRD = build_backbone(arrd)
        self.sr_loss = nn.L1Loss()
        self.scale_list = scale_cfg['scale_list']
        self.deg_type = scale_cfg['deg_type']


    def deg_resolution(self, img, img_meta, l_size, gt_bbox):
    ...
    def extract_feat(self, img_lr):
        ...
        if self.with_neck:
            x_l, x_s = self.neck(x_l)
        x_sr = self.ARRD(x_s)
        return [x_l], x_sr
        #return [torch.cat([x_l, x_s], dim=1)], x_sr
    ...
    def forward_train(self,
        ...
        #print(img.shape)
        l_size = random.choice(self.scale_list) * 32 # Must be Multiples of 32, 128 -- 512
        # img_lr = torch.empty(size=(batch_size, 3, l_size, l_size)).to(torch.device(device))
        imgs_down = []
        gt_bboxes_down = []
        
        # Resolution
        if self.deg_type == 'resolution':
            for i in range(batch_size):
                img_down, gt_bbox_down = deg_resolution(img[i], img_metas[i], l_size, gt_bboxes[i])
                gt_bboxes_down.append(gt_bbox_down)
                imgs_down.append(img_down)
        # Noise & Blur
        elif self.deg_type == 'nb':
            for i in range(batch_size):
                img_down, gt_bbox_down = random_deg(img[i], img_metas[i], l_size, gt_bboxes[i],keep_shape=True)
                gt_bboxes_down.append(gt_bbox_down)
                imgs_down.append(img_down)
        # Default (ALL) Noise & Blur & Resolution
        elif self.deg_type == 'all':
            for i in range(batch_size):
                img_down, gt_bbox_down = random_deg(img[i], img_metas[i], l_size, gt_bboxes[i],keep_shape=False)
                gt_bboxes_down.append(gt_bbox_down)
                imgs_down.append(img_down)

        imgs_down = torch.stack(imgs_down, dim=0)
        x, img_sr = self.extract_feat(imgs_down)
        losses = self.bbox_head.forward_train(x, img_metas, gt_bboxes_down,
                                                gt_labels, gt_bboxes_ignore)
                                                
        losses['sr_loss'] = 0.4 * self.sr_loss(img_sr, img)
    ...
    def simple_test(self, img, img_metas, rescale=False):
        ...
        """
        feat, sr_out = self.extract_feat(img)
    ...
    def aug_test(self, imgs, img_metas, rescale=True):
        ...
        for ind, flip_ind in zip(img_inds[0::2], img_inds[1::2]):
            ...
            x, sr_out = self.extract_feat(img_pair)
